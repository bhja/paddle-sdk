...
public class BusinessService extends ApiResource<Business> {
    ...
    public Business createBusiness(String customerId, BusinessCreateParams params) throws PaddleException {
        try {
            HttpRequest request = httpClient().request(URI.create(String.format("%s/%s/%s", baseUrl(), CUSTOMERS, customerId, BUSINESS)),
    ...
    public Business getBusiness(String customerId, String businessId) throws PaddleException {
        HttpRequest request = httpClient().request(
                URI.create(String.format("%s/%s/%s/%s", baseUrl(), CUSTOMERS, customerId, BUSINESS, businessId)), HttpMethod.GET.name(),
