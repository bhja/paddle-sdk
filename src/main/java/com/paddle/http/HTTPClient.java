...
public class HTTPClient {
    ...
    public HTTPClient(HTTPConfig config) {
        client = HttpClient.newBuilder().version(HttpClient.Version.HTTP_2).followRedirects(HttpClient.Redirect.NORMAL).connectTimeout(
                Duration.ofSeconds(config.getConnectionTimeOut() > 0 ? config.getConnectionTimeOut() : DEFAULT_CONNECT_TIMEOUT)).build();
    ...
    public HttpRequest request(URI uri, Map<String, String> headers, String method, HttpRequest.BodyPublisher bodyPublisher) {
    ...
    public HttpResponse<String> execute(HttpRequest request, HttpResponse.BodyHandler<String> bodyHandler)
            throws InterruptedException, IOException {
        return client.send(request, bodyHandler);
    }

    private Map<String, String> defaultHeaders() {
        return Map.of(AUTHORIZATION, BEARER + this.apiKey, CONTENT_TYPE, APPLICATION_JSON, ACCEPT, APPLICATION_JSON);
